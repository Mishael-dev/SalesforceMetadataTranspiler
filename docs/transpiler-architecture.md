...
### 3. **MetadataConverter**
Converts validated JSON to internal metadata representation.

```typescript
class MetadataConverter {
  private objectConverter: ObjectMetadataConverter;
  private fieldConverters: Map<FieldType, FieldMetadataConverter>;
  
  convert(validatedSchema: ValidatedSchema): MetadataStructure {
    const metadataObjects = validatedSchema.objects.map(obj => {
      return {
        objectMetadata: this.objectConverter.convert(obj),
        fieldMetadata: this.convertFields(obj.fields)
      };
    });
...
#### **3a. ObjectMetadataConverter**
Converts custom object JSON to object metadata structure.

```typescript
class ObjectMetadataConverter {
  convert(objectJson: CustomObjectSchema): ObjectMetadata {
    return {
      fullName: objectJson.fullName,
      label: objectJson.label,
      pluralLabel: objectJson.pluralLabel,
      deploymentStatus: objectJson.deploymentStatus,
      sharingModel: objectJson.sharingModel,
      nameField: this.convertNameField(objectJson.nameField),
      features: objectJson.features,
      // ... other properties
    };
  }
  
  private convertNameField(nameField: NameField): NameFieldMetadata {}
}
```

#### **3b. FieldMetadataConverter (Abstract Base)**
Abstract base class for field-specific converters.

```typescript
abstract class FieldMetadataConverter {
  abstract convert(field: Field): FieldMetadata;
  
  protected convertCommonFields(field: Field): CommonFieldMetadata {
    return {
...
class TextFieldConverter extends FieldMetadataConverter {
  convert(field: TextField): FieldMetadata {
    return {
      ...this.convertCommonFields(field),
      length: field.length
    };
  }
}

class PicklistFieldConverter extends FieldMetadataConverter {
  convert(field: PicklistField): FieldMetadata {
    return {
      ...this.convertCommonFields(field),
      valueSet: this.convertValueSet(field.valueSet)
    };
  }
  
  private convertValueSet(valueSet: ValueSet): ValueSetMetadata {}
}

class LookupFieldConverter extends FieldMetadataConverter {
  convert(field: LookupField): FieldMetadata {
    return {
      ...this.convertCommonFields(field),
...
class MasterDetailFieldConverter extends FieldMetadataConverter {
  convert(field: MasterDetailField): FieldMetadata {
    return {
      ...this.convertCommonFields(field),
      referenceTo: field.referenceTo,
      relationshipName: field.relationshipName,
...
// ... More field converters (Formula, Summary, AutoNumber, etc.)
```
